...
public:
    ...
    void defaults()
    {
        ...
        transmitDevTryMax(6);
        updIntervall(PARAMETER_UPDATE_CYCLE);
        altitude(PARAMETER_ALTITUDE_ABOVE_SEALEVEL);            // height of sensor's location above sea level
    }
};
...
public:
    ...
    virtual void trigger(AlarmClock& clock)
    {
...
#ifdef ADS1115
        msg.init(msgcnt, temperature10, carbondioxide100, humidity+OFFSEThumi, operatingVoltage1000, device().battery().low(), batteryVoltage1000, VCCVoltage1000);
#else
        msg.init(msgcnt, temperature10, carbondioxide100, humidity+OFFSEThumi, operatingVoltage1000, device().battery().low());
#endif
        ...
        set(seconds2ticks(max(updCycle,PARAMETER_UPDATE_CYCLE)));
        CLOCK.add(*this);
        regularWakeUp = true;
    }
...
#ifdef SENSOR_SCD30
        scd30.init(PARAMETER_ALTITUDE_ABOVE_SEALEVEL, PARAMETER_AMBIENT_PRESSURE, OFFSETtemp, PARAMETER_UPDATE_CYCLE);
#endif
